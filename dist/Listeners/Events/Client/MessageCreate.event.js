...
export default class MessageCreate extends Event {
    ...
    async run(_client, message) {
        ...
        if (getData === true) {
            ...
            message.reply({
                content: `Welcome back **${message.author.username}**! You went afk <t:${Math.floor(timestamp / 1000)}:R> with the reason - **${reason}**`,
                allowedMentions: { parse: ["users"] },
            });
        }
...
Server I'd : \`${message.guild.id}\``);
        ...
        const prefixRegex = new RegExp(`^(<@!?${this.client.user.id}>|${escapeRegex(prefix)}${hasnp ? "|" : ""})\\s*`);
                    ...
                    i.reply({
                        embeds: [
                            this.client
                                .embed()
                                .setColor(this.client.color.main)
                                .setTitle(`[Help for command: ${command.name}](https://discord.gg/9bWCU6VPEM)`)
                                .setFields(),
                        ],
                    });
                    break;
