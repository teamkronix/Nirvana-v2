...
export default class InteractionCreate extends Event {
    ...
    async run(client, interaction) {
        if (interaction instanceof CommandInteraction &&
            ...
            if (!command)
                return;
            if (await this.client.db.isBlacklisted(interaction.user.id, interaction.guild.id))
                return;
            ...
            if (command.player) {
                if (command.player.voice) {
                    ...
                    if (interaction.guild.members.resolve(this.client.user).voice.channel) {
                        if (interaction.guild.members.resolve(this.client.user).voice.channelId !==
                            interaction.member.voice.channelId)
                            return await interaction.reply({
                                content: `You are not connected to <#${interaction.guild.members.resolve(this.client.user).voice.channel.id}> to use \`${command.name}\` command.`,
                                ephemeral: true,
                            });
                    }
                }
