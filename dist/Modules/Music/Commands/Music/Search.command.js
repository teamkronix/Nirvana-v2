    ...
    async run(client, ctx, args) {
        ...
        switch (res.loadType) {
            ...
            case LoadType.SEARCH: {
                ...
                const SearchResults = tracks.map((track, index) => `\`[${index + 1}]\` | [${track.info.title}](${track.info.uri}) (\`${this.client.botfunctions.formatTime(track.info.length)}\`)`);
        ...
        collector.on("collect", async (int) => {
            ...
            await ctx.editMessage({
                embeds: [
                    embed
                        .setTitle(null)
                        .setDescription(`Added [${song.info.title}](${this.client.config.url.support}) by [${song.info.author}](${this.client.config.url.support}) to the queue`),
                ],
                components: [
                    {
                        type: 1,
